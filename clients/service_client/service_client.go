...
import (
	"encoding/json"
	"errors"
	"github.com/vox-ai/nacos-go/clients/nacos_client"
	"github.com/vox-ai/nacos-go/common/constant"
	"github.com/vox-ai/nacos-go/common/http_agent"
	"github.com/vox-ai/nacos-go/common/nacos_error"
	"github.com/vox-ai/nacos-go/common/util"
	"github.com/vox-ai/nacos-go/vo"
	"io/ioutil"
	"log"
	"net/http"
	"strconv"
...
func (client *ServiceClient) LogoutServiceInstance(param vo.LogoutServiceInstanceParam) (success bool, err error) {
	...
	if err == nil && len(param.Cluster) <= 0 {
		err = errors.New("[client.LogoutServiceInstance] param.ClusterName can not be empty")
	}
	var clientConfig constant.ClientConfig
	var serverConfigs []constant.ServerConfig
	var agent http_agent.IHttpAgent
...
func (client *ServiceClient) ModifyServiceInstance(param vo.ModifyServiceInstanceParam) (success bool, err error) {
	...
	if err == nil && len(param.Cluster) <= 0 {
		err = errors.New("[client.ModifyServiceInstance] param.ClusterName can not be empty")
	}
	var clientConfig constant.ClientConfig
	var serverConfigs []constant.ServerConfig
	var agent http_agent.IHttpAgent
...
func (client *ServiceClient) beatTask(clientConfig constant.ClientConfig,
	...
	//		log.Println(err)
	//	}
	//	//if serviceDetail.ServiceInfo.HealthCheckMode != "client" {
